...
export async function GET(request: Request){
  ...
  for(const l of links){
    const t = l.trade; const pnl = computeRealizedPnl({ entryPrice:t.entryPrice, exitPrice:t.exitPrice??undefined, quantity:t.quantity, direction:t.direction, fees:t.fees, contractMultiplier:(t as any).instrument?.contractMultiplier });
  ...
  if(format==='xlsx'){
    ...
    return new Response(new Uint8Array(out as any),{status:200, headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','Content-Disposition':'attachment; filename="tag-performance.xlsx"'}});
  }
  const headers = ['tagId','label','trades','wins','losses','winRate','sumPnl','avgPnl'];
  const stream = rowsToCsvStream(headers, rows as any);
